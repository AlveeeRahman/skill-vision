...
class QualityScorer:
    ...
    def assess_quality(self) -> QualityReport:
        ...
        try:
            ...
            # Skill-type awareness. Documentation and router skills legitimately ship no
            # Python, so scoring a Code Quality dimension they can never earn imposed a
            # hard 75-point ceiling on perfectly good skills. When there are no scripts,
            # that dimension is dropped and its weight is redistributed across the
            # dimensions that do apply.
            scripts_dir = self.skill_path / "scripts"
            has_scripts = any(
                q for q in scripts_dir.rglob("*.py") if "__pycache__" not in q.parts
            ...
            if has_scripts:
                ...
                n = 4 if self.include_security else 3
                w = round(1.0 / n, 4)
                self._score_documentation(w)
                self._score_completeness(w)
                self._score_usability(w)
                if self.include_security:
                    self._score_security(w)
    ...
    def _score_skill_md(self, dimension: QualityDimension):
        ...
        try:
            ...
            # The body loads into context on every trigger, so the spec caps it at ~500
            # lines. Scoring longer as better rewarded exactly the bloat the guidance
            # warns against; a concise SKILL.md that answers the question is the goal.
            if line_count > 500:
                length_score = 10          # over the spec ceiling
            elif line_count >= 20:
                length_score = 25          # comfortably within budget
            elif line_count >= 8:
                length_score = 18          # terse but plausible
            else:
                length_score = 5           # too thin to guide anything

            dimension.add_score("skill_md_length", length_score, 25,
                               f"SKILL.md has {line_count} lines "
                               f"({'over' if line_count > 500 else 'within'} the 500-line ceiling)")

            if line_count > 500:
                dimension.add_suggestion(
                    f"SKILL.md is {line_count} lines, past the ~500-line ceiling. Move detail "
                    f"into references/ — the whole body loads on every trigger.")
    ...
    # Both spellings occur in the wild: the house standard says "references",
    # but vendored/upstream skills commonly ship "reference". The Agent Skills
    # spec mandates neither, so accept either rather than scoring a skill as
    # undocumented over one character.
    REFERENCE_DIR_NAMES = ("references", "reference")

    def _find_reference_dir(self) -> Optional[Path]:
        """Return the skill's reference directory under either accepted name."""
        for dir_name in self.REFERENCE_DIR_NAMES:
            candidate = self.skill_path / dir_name
            if candidate.is_dir():
                return candidate
        return None
    ...
    def _score_references(self, dimension: QualityDimension):
        ...
        references_dir = self._find_reference_dir()
      ...
      # rglob, not glob: skills that namespace files into subdirectories
        # (scripts/<area>/x.py, references/<area>/y.md) reported zero and were
        # graded as if they had none.
        ref_files = ([p for p in references_dir.rglob("*.md")]
                     + [p for p in references_dir.rglob("*.txt")])
    ...
    def _score_code_quality(self, weight: float = 0.25):
        ...
        python_files = [p for p in scripts_dir.rglob("*.py")
    ...
    def _score_help_accessibility(self, dimension: QualityDimension):
        ...
        doc_files = ([p for p in self.skill_path.glob("*.md")]
                     + [p for p in (self.skill_path / "guides").rglob("*.md")
                        if (self.skill_path / "guides").is_dir()])
        if len(doc_files) >= 2:
    ...
    def _score_security(self, weight: float = 0.20):
        ...
        try:
            ...
            # Extract scores from SecurityScorer result. The four component
            # scores are bare floats nested under "components" -- reading them
            # from the top level with a .get() default silently yielded 0 for
            # every skill, which reads as "measured and failed" rather than
            # "never measured". Raise instead of defaulting if the shape drifts.
            try:
                components = result["components"]
                sensitive_data_score = components["sensitive_data_exposure"]
                file_ops_score = components["safe_file_operations"]
                command_injection_score = components["command_injection_prevention"]
                input_validation_score = components["input_validation"]
            except (KeyError, TypeError) as shape_error:
                raise RuntimeError(
                    f"SecurityScorer returned an unexpected result shape: {shape_error}. "
                    f"Expected a 'components' dict with the four component scores."
                ) from shape_error
            ...
            # Add suggestions from SecurityScorer. The module emits "suggestions"
            # (remediation advice) and "findings" (concrete hits); there is no
            # "issues" key, so the previous lookup always yielded nothing.
            for suggestion in result.get("suggestions", []):
                dimension.add_suggestion(suggestion)
