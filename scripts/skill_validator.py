...
class SkillValidator:
    ...
    def _validate_skill_md(self):
        ...
        try:
            ...
            # Check line count
            # Spec guidance caps SKILL.md at ~500 lines; there is no minimum. A short
            # SKILL.md is a virtue, not a defect, so falling under the house floor is a
            # note rather than an error. Exceeding the ceiling is the real problem.
            SPEC_MAX_LINES = 500
            if line_count > SPEC_MAX_LINES:
                self.report.add_check("skill_md_length", False,
                                     f"SKILL.md has {line_count} lines (>{SPEC_MAX_LINES} spec ceiling)", 0.0)
                self.report.add_error(
                    f"SKILL.md too long: {line_count} lines, spec guidance is under {SPEC_MAX_LINES}")
            ...
            if line_count >= min_lines:
                ...
                self.report.add_check("skill_md_length", False,
                                     f"SKILL.md has {line_count} lines (<{min_lines} house target)", 0.5)
                self.report.add_suggestion(
                    f"SKILL.md is {line_count} lines, below the {min_lines}-line house target. "
                    f"This is a house-style preference, not a spec requirement — do not pad "
                    f"a concise skill to satisfy it.")
        ...
        if content.startswith('---'):
            try:
                ...
                if not missing_fields:
                    ...
                    self.report.add_suggestion(
                f"House-style frontmatter fields absent: {', '.join(missing_fields)}. "
                f"The Agent Skills spec requires only `name` and `description`; these extra "
                f"fields are this repository's convention, not a portability requirement.")
    ...
    def _validate_required_sections(self, content: str):
        ...
        if not missing_sections:
            ...
            self.report.add_suggestion(
                f"House-style sections absent: {', '.join(missing_sections)}. "
                f"The spec places no constraints on body structure.")
    ...
    def _validate_directory_structure(self):
        ...
        # Detect skill type so the structure rules fit the skill. The spec makes
        # scripts/, references/ and assets/ optional, and a documentation-only or
        # router skill legitimately ships no scripts at all. Grading those against a
        # tool-skill checklist produced most of this validator's false failures.
        has_scripts = any((self.skill_path / "scripts").rglob("*.py")) \
            if (self.skill_path / "scripts").is_dir() else False
        guide_count = len(list((self.skill_path / "guides").glob("*.md"))) \
            if (self.skill_path / "guides").is_dir() else 0
        if guide_count >= 2:
            self.skill_kind = "router"
        elif has_scripts:
            self.skill_kind = "tool"
        else:
            self.skill_kind = "documentation"

        if self.skill_kind in ("documentation", "router"):
            # Only structural directories the skill actually uses are graded.
            required_dirs = [d for d in required_dirs
                             if (self.skill_path / d).is_dir()]
            self.report.add_check("skill_kind", True,
                                 f"Detected skill type: {self.skill_kind} "
                                 f"(scripts/ not required)", 1.0)
        
        # Check required directories
        missing_required = []
        # The spec makes scripts/, references/ and assets/ all optional. A
        # documentation-only skill with no scripts is valid, so a missing directory is
        # reported as a suggestion rather than a failure.
    ...
    def _validate_python_scripts(self):
        ...
        # rglob, not glob: skills that namespace scripts into subdirectories
        # (scripts/<tool>/foo.py) previously reported "0 scripts" and had every
        # script-quality check silently skipped.
        python_files = sorted(
    ...
    def _validate_single_script(self, script_path: Path):
        ...
        try:
            ...
            try:
                ...
                self._validate_script_features(tree, script_name, content, script_path)
    ...
    def _validate_script_features(self, tree: ast.AST, script_name: str, content: str, script_path=None):
        """Validate required script features"""
        required_features = self._get_tier_requirement("features_required", ["argparse", "main_guard"])

        # Unit-test modules are run by a test runner, not invoked with flags, so
        # the CLI-surface requirements do not apply to them. Flagging a passing
        # test file as "must use argparse" penalised skills for having tests.
        is_test_module = script_name.startswith("test_") or script_name.endswith("_test.py")
        if is_test_module:
            required_features = [f for f in required_features
                                 if f not in ("argparse", "json_output", "help_text")]
        ...
        if "main_guard" in required_features:
            # Detect via AST, not substring: `if __name__ == '__main__':` (single quotes)
            # and spacing variants are equally valid and were previously reported as
            # missing, failing scripts that have a working guard.
        ...
        external_imports = self._check_external_imports(tree, script_path)
        if not external_imports:
            ...
            self.report.add_suggestion(
                f"{script_name} imports third-party packages: {', '.join(external_imports)}. "
                f"That is fine when the skill is about those libraries — just make sure they "
                f"are named in `compatibility` or the install instructions.")
    ...
    def _check_external_imports(self, tree: ast.AST, script_path=None) -> List[str]:
        """Check for external (non-stdlib) imports"""
        # Prefer the interpreter's authoritative list (Python 3.10+). The hand-maintained
        # set below is a fallback for older interpreters; it omitted `__future__` and many
        # common modules (asyncio, inspect, importlib, zipfile, ...), which produced false
        # "external import" failures on scripts that were pure stdlib.
        stdlib_modules = set(getattr(sys, "stdlib_module_names", ())) | {
            ...
            '__future__', 'asyncio', 'inspect', 'importlib', 'zipfile', 'tarfile',
            'gzip', 'shlex', 'signal', 'socket', 'ssl', 'select', 'configparser',
            'concurrent', 'numbers', 'binascii', 'zlib', 'stat', 'ipaddress',
            'graphlib', 'contextvars', 'builtins', 'keyword', 'tokenize',
        }
        
        # Modules sitting next to this script are local project code, not third-party
        # dependencies. Without this, `from security_scorer import ...` in a sibling file
        # was reported as an external import.
        local_modules = set()
        if script_path is not None:
            try:
                script_dir = Path(script_path).parent
                for sibling in script_dir.glob("*.py"):
                    local_modules.add(sibling.stem)
                for pkg in script_dir.iterdir():
                    if pkg.is_dir() and (pkg / "__init__.py").exists():
                        local_modules.add(pkg.name)
            except OSError:
                pass
