...
## entry-012: Demote the .git JUNK_FILES warning to an informational note

- **timestamp**: 2026-08-17
- **task_type**: cleanup
- **instruction**: Stop `spec_validator.py` from warning about `.git` on every fresh clone — demote it to an informational note with actionable packaging advice — and silence the deprecation warning `quality_scorer.py` prints under modern Python.
- **context**: Every user who cloned the repo and ran `python3 scripts/spec_validator.py .` saw `JUNK_FILES: .git` in the warning list. The warning was technically honest — a naive `zip -r` for claude.ai upload WOULD leak the `.git` directory into the package — but it fired on 100% of cloned skills, so it was permanent noise that trained users to ignore the warning channel. Separately, `scripts/quality_scorer.py` used the deprecated `datetime.utcnow()`, printing a `DeprecationWarning` to stderr on every run under Python 3.12+.
- **action_taken**: In `scripts/spec_validator.py` `check_packaging_hygiene`: paths containing `.git` are now skipped by the junk scan (setting a `has_git` flag) and emit a dedicated INFO note `GIT_DIR` ("A .git directory is present (normal for a cloned skill)") whose remediation string gives the exact exclusion command: `zip -r skill.zip . -x '.git/*' '.git'`. Two new tests in `tests/test_spec_validator.py`: `test_git_dir_is_note_not_junk_warning` (a skill with only `.git` gets GIT_DIR, not JUNK_FILES) and `test_git_dir_does_not_mask_real_junk` (a skill with `.git` AND a `__pycache__` `.pyc` still gets the JUNK_FILES warning alongside GIT_DIR). In `scripts/quality_scorer.py`: `datetime.utcnow().isoformat() + "Z"` replaced with `datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")`, importing `timezone`.
- **diff_summary**: `scripts/spec_validator.py` +11 lines (skip-and-flag in the junk loop, new GIT_DIR info emission); `tests/test_spec_validator.py` +15 lines (2 new tests, suite 93 → 95); `scripts/quality_scorer.py` +2/−2 (import and timestamp construction). Fresh-clone validator output improved from `0 errors, 1 warnings, 4 notes` to `0 errors, 0 warnings, 5 notes`.
- **rationale**: The signal-vs-noise fix keeps the honesty (`.git` in a zip upload is still a real leak) but moves it to the severity tier that matches its base rate — a condition true of every legitimate clone cannot be a warning; putting the exact `zip -x` command in the note makes the INFO actionable rather than nagging. The masking test exists because the naive fix (`continue` on `.git` paths) could plausibly have been written to skip the whole junk scan; the timestamp fix uses `timezone.utc` rather than suppressing the warning because the API is deprecated for a reason.
- **verification**: `/Users/alverahmanakash/Desktop/dev/metal_env/bin/python -m pytest tests -q` → `95 passed`; `python3 scripts/spec_validator.py .` → `CONFORMANT (0 errors, 0 warnings, 5 notes)` with the GIT_DIR note and its zip-exclusion remediation visible in the output; `python3 scripts/quality_scorer.py assets/sample-skill` runs with no DeprecationWarning on stderr.
- **likely_errors**: The `.git` skip accidentally swallowing detection of real junk that happens to sit alongside it (guarded by `test_git_dir_does_not_mask_real_junk`); matching `.git` by substring instead of path-part so files like `foo.github.md` get skipped; demoting a warning without updating tests that asserted the old severity; timezone-aware `isoformat()` emitting `+00:00` instead of the `Z` suffix consumers may parse.
- **outcome**: success
- **lesson**: A warning that fires on 100% of legitimate cases is noise, not signal — demote it to a note and put the exact remediation command in the note's text.

## entry-013: Retarget the README at Claude and Claude Code users

- **timestamp**: 2026-08-17
- **task_type**: docs
- **instruction**: Audit the inherited README against the actual tools, delete or fix everything that is fabricated or monorepo-legacy, and restructure it around how Claude/Claude Code users actually install and run a skill.
- **context**: An audit found real defects in the inherited README: a fabricated `--batch` flag in the Advanced Usage example (`scripts/quality_scorer.py` has no such flag — verified via `--help`); monorepo-legacy `engineering/**` paths throughout the CI, pre-commit, and batch examples that make no sense in a standalone repo; a Getting Started section that duplicated Quick Start; and an unverified exit-code claim, where the real, tested codes are: skill_validator 0/1/130; quality_scorer 0/1/2/130 (2 = grade D); script_tester 0/1/2/130 (2 = partial).
- **action_taken**: Rewrote `README.md`: new "Install as a Claude skill" section based on doc-verified mechanics — Claude Code personal (`~/.claude/skills/`) and project (`.claude/skills/`) installs via `git clone`; claude.ai/Desktop zip upload requiring exactly one top-level folder and excluding `.git`; the claude.ai 200-character limit on the frontmatter description versus this repo's 460-character description, with a ready-to-paste short alternative; Claude API beta skills endpoints and the no-network container that stdlib-only satisfies. Merged Getting Started + Quick Start into one "Using the validators" section covering all five tools with generic paths; added a verified exit-code table ("safe to build CI gates on"); rewrote the CI recipe for a generic `skills/` layout that clones skill-pirate inside the workflow; deleted the `--batch` example; reordered sections to human logic (try → install → how → why → use → output → gate). Updated the 93 → 95 test-count references in `SKILL.md` and `CONTRIBUTING.md`.
- **diff_summary**: `README.md` +~200/−~240 lines (429 lines touched, net shorter); `SKILL.md` and `CONTRIBUTING.md` one line each (test count 93 → 95). No script behavior changed in this event.
- **rationale**: The audience that will actually find this repo is Claude/Claude Code users installing it as a skill, so install mechanics lead; every surviving claim (flags, paths, exit codes, description-length limits) was verified against `--help` output, test assertions, or platform docs rather than inherited — the fabricated `--batch` flag showed the inherited text could not be trusted claim-by-claim; monorepo `engineering/**` paths were replaced with a layout a standalone user could copy unmodified.
- **verification**: `python3 scripts/spec_validator.py .` → `CONFORMANT (0 errors, 0 warnings, 5 notes)`; `/Users/alverahmanakash/Desktop/dev/metal_env/bin/python -m pytest tests -q` → `95 passed`; `python3 scripts/quality_scorer.py --help` confirms no `--batch` flag exists. Mid-rewrite, the validator itself caught a regression: dropping the old links to `assets/sample-skill/README.md` and `assets/sample-skill/references/api-reference.md` produced two ORPHANED_REFERENCE warnings — fixed by relinking both files from the new "What's in the box" section before finishing.
- **likely_errors**: Documenting flags or exit codes from memory instead of the tools' actual behavior (the inherited `--batch` example is exactly this failure); deleting prose that was silently load-bearing for the link graph, orphaning bundled files (happened mid-rewrite, caught by the validator's ORPHANED_REFERENCE check); example paths that only work in the repo they were copied from; stale numeric claims (test counts, description lengths) scattered across multiple docs drifting out of sync.
- **outcome**: success
- **lesson**: Docs claims are code claims — verify every flag, path, and exit code against the tools themselves before publishing, and let the validator's link checks run over every draft.

## entry-014: Reposition as a Claude Code skill; drive validator self-output to zero noise

- **timestamp**: 2026-08-17
- **task_type**: docs
- **instruction**: Reposition the project as what it actually is — a QA skill for Claude Code, not a generic CLI toolkit — strip the remaining structural slop from the README, and fix every remaining validator finding at its cause so the self-validation demo is genuinely clean.
- **context**: Same uncommitted working tree as entries 012–013, evolved further before the single planned commit — this entry supersedes parts of the README state logged in entry-013. The README still led with CLI usage, carried an 18-entry Table of Contents inherited from the pushed version, duplicated its "Why" bullets in an all-✅/❌ "At a glance" table, and padded itself with "Quality standards", "Requirements", and separate "Advanced usage"/"Error handling"/"Output formats"/"Self-testing" headers. Fresh-clone validator output still carried four INFO notes, two of them fixable (REFERENCE_NO_TOC on `references/validator-comparison.md`; two UNREACHABLE_FROM_SKILL_MD notes for the sample-skill docs). Marketing a Claude-dependent skill as a generic CLI tool was the wrong identity.
- **action_taken**: (1) Identity reframe on the owner's direction: README H1 is now "Skill Pirate — the QA skill for Claude Code"; the page leads with skill installation (`git clone` into `~/.claude/skills/` for personal or `.claude/skills/` for project scope) and natural-language prompts to Claude ("Validate `./my-skill` against the Agent Skills spec", "Score the quality of my skill…"), with the five CLIs demoted to an "Under the hood: the five validators" section framed as what Claude runs for you — and what CI can run without Claude. (2) De-slop: removed the "At a glance" comparison table (verbatim redundancy with the Why bullets above it) and the four filler headers; exit codes and CI/pre-commit recipes now collapse inside Under the hood; final structure is 11 sections with no Table of Contents. (3) Noise-to-zero at the cause: added a Contents list to `references/validator-comparison.md`; linked `assets/sample-skill/README.md` and `assets/sample-skill/references/api-reference.md` from `SKILL.md` directly; regenerated the README's self-validation demo block from the genuine fresh-clone output.
- **diff_summary**: Versus the committed README: 399 lines touched, net roughly −160 (156 insertions / 315 deletions across the working tree); `references/validator-comparison.md` +8 lines (Contents); `SKILL.md` +2 lines (sample-skill links). Fresh-clone validator output: 4 notes → 2 notes, both inherent — DEEP_NESTING is the demo skill's legitimate three-level structure, GIT_DIR is true of any clone.
- **rationale**: A skill whose value is delivered through Claude Code should be positioned by that dependency — leading with CLI invocations buried the actual product; the redundant table and filler headers were deleted rather than reworded because their content already existed elsewhere on the page; the two fixable validator notes were fixed at the source (a real TOC, real links from SKILL.md) instead of trimming them from the demo output, because a doctored demo would be exactly the dishonesty the tool exists to catch; at 11 sections a TOC is navigation for a problem the page no longer has.
- **verification**: `python3 scripts/spec_validator.py .` → `CONFORMANT (0 errors, 0 warnings, 2 notes)`, only DEEP_NESTING and GIT_DIR remaining, and the README's fenced demo block now matches this output verbatim; `/Users/alverahmanakash/Desktop/dev/metal_env/bin/python -m pytest tests -q` → `95 passed`; `grep` over the README confirms 11 section headers and no Table of Contents.
- **likely_errors**: Repositioning docs without updating every section that still speaks the old identity (a stray "CLI tool" framing surviving the rewrite); deleting "redundant" sections whose links were load-bearing for the validator's reachability graph (the entry-013 orphan regression is the precedent); silencing validator findings by editing the demo output instead of the cause, which the next fresh clone exposes; adding a TOC to a reference doc with anchors that do not match its heading slugs.
- **outcome**: success
- **lesson**: Position a product by its dependency — a skill for Claude Code, not a CLI that happens to include a SKILL.md — and fix validator findings at the cause rather than trimming them from the demo output.
