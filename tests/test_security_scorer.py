...
""")
    ...
    def test_overall_score_is_sum_of_components(self):
        """Overall score is the sum of the four components, on a 0-100 scale.

        This previously asserted a 0.25-weighted average, which capped the
        result at MAX_COMPONENT_SCORE (25) and made both the 70/50 tier
        thresholds and the 30-point critical-vulnerability cap unreachable.
        """
        with tempfile.TemporaryDirectory() as tmpdir:
            script_path = Path(tmpdir) / "test.py"
            script_path.write_text("""
...
""")
            ...
            expected = sum(results['components'].values())
...
""")
            ...
            # Secure script should score reasonably well. The bar is 70/100,
            # which preserves the intent of the original 20/25 assertion now
            # that overall_score is on a 0-100 scale.
